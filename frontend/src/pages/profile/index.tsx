...
export default function ProfilePage() {
  ...
  const updateProfile = useMutation({
    mutationFn: async () => {
      const res = await authFetch("/api/v1/users/me", { method: "PATCH", body: JSON.stringify(form) });
  ...
  const changePassword = useMutation({
    mutationFn: async () => {
      ...
      const res = await authFetch("/api/v1/users/me", { method: "PATCH", body: JSON.stringify({ currentPassword: pwForm.currentPassword, newPassword: pwForm.newPassword }) });
      const json = await res.json();
      if (!json.success) throw new Error(json.error);
    },
