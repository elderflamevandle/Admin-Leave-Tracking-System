...
export default function LeaveManagePage() {
  ...
  const pendingParam = isManager ? "/api/v1/leave?team=true&status=pending&limit=50" : "/api/v1/leave?all=true&status=pending&limit=50";
  const allParam = isManager ? "/api/v1/leave?team=true&limit=50" : "/api/v1/leave?all=true&limit=50";
  ...
  const approveMutation = useMutation({
    mutationFn: async (id: string) => {
      const res = await authFetch(`/api/v1/leave/${id}/approve`, { method: "POST" });
  ...
  const rejectMutation = useMutation({
    mutationFn: async ({ id, note }: { id: string; note: string }) => {
      const res = await authFetch(`/api/v1/leave/${id}/reject`, { method: "POST", body: JSON.stringify({ note }) });
      const json = await res.json();
      if (!json.success) throw new Error(json.error);
    },
    onSuccess: () => {
