...
export default function LeaveCalendarPage() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const res = await authFetch(`/api/leave/calendar?month=${monthKey}`);
  ...
  const getHolidayForDay = (day: number) => {
    ...
    return holidays.find((h) => {
      if (h.isRecurring) {
        // h.date may be a full ISO string ("2026-01-01T00:00:00.000Z") or a date
        // string ("2026-01-01") — slice(5,10) gives "MM-DD" in both cases.
