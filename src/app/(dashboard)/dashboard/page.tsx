  ...
  const { data: stats } = useQuery({
    ...
    queryFn: async () => {
      const [usersRes, leaveRes, reportRes] = await Promise.all([
        ...
        authFetch("/api/leave?all=true&status=pending&limit=50"),
        authFetch(`/api/admin/reports?year=${new Date().getFullYear()}`),
      ]);
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <Button asChild variant="outline" size="sm">
          <Link href="/admin/reports"><BarChart2 className="mr-2 h-4 w-4" />Reports</Link>
        </Button>
      </div>
      ...
      <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
        ...
        <Card>
          <CardHeader className="pb-2">
            <CardTitle className="text-sm font-semibold flex items-center justify-between">
              Pending Leave Requests
              <Link href="/leave/manage" className="text-xs text-blue-600 font-normal hover:underline">View all →</Link>
            </CardTitle>
          </CardHeader>
          <CardContent>
        ...
        <Card>
          ...
          <CardContent className="space-y-2">
            {[
              { href: "/leave/manage",    label: "Review Leave Requests",  icon: <CalendarOff className="h-4 w-4" /> },
              { href: "/leave/calendar",  label: "Leave Calendar",         icon: <Clock className="h-4 w-4" /> },
              { href: "/admin/users",     label: "Manage Employees",       icon: <Users className="h-4 w-4" /> },
              { href: "/admin/reports",   label: "Analytics & Reports",    icon: <BarChart2 className="h-4 w-4" /> },
              { href: "/admin/holidays",  label: "Manage Holidays",        icon: <XCircle className="h-4 w-4" /> },
            ].map(({ href, label, icon }) => (
              <Link key={href} href={href} className="flex items-center gap-2 rounded-md px-3 py-2 text-sm text-slate-700 hover:bg-slate-100 transition-colors">
...
function ManagerDashboard() {
  ...
  const { data: teamLeave } = useQuery({
    ...
    queryFn: async () => {
      const res = await authFetch("/api/leave?team=true&status=pending&limit=50");
      return res.json();
    },
  });
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex gap-3">
        <Button asChild variant="outline" size="sm">
          <Link href="/leave/manage">Review Team Leave</Link>
        </Button>
        <Button asChild variant="outline" size="sm">
...
function EmployeeDashboard() {
  ...
  const { data: timeData } = useQuery({
    ...
    queryFn: async () => {
      const res = await authFetch("/api/timelog?limit=7");
      return res.json();
    },
  });
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex gap-3">
        <Button asChild variant="outline" size="sm">
          <Link href="/leave">Apply for Leave</Link>
        </Button>
        <Button asChild variant="outline" size="sm">
          <Link href="/timelog">Log Time</Link>
        </Button>
        <Button asChild variant="outline" size="sm">
          <Link href="/leave/calendar">Leave Calendar</Link>
        </Button>
